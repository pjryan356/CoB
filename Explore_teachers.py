...
custom_palette = sns.color_palette([rc.RMIT_Blue, rc.RMIT_White, rc.RMIT_Green])
...
def get_course_teacher_ces(cur, tbl='vw811_teacher_data', schema='ces'):
  ...
  qry = ' SELECT \n' \
        ...
        ' FROM {0}.{1} t1 \n' \
        " WHERE t1.teacher_count > 5 AND type != 'Z' \n" \
...
for i in range(0, step*19+1, step):
  ...
  sns.boxplot('emplid', 'gts', ax=ax,
              data=df.loc[(df['emplid'].isin(teachers))],
              order=teachers, hue='type', hue_order=['Staff', 'A', 'Course'], palette=custom_palette,
  ...
  l = plt.legend([handles[0], handles[-1], handles[2]], ['Teacher', 'Teacher 2019', 'Course Whole'], loc='lower right')
  #x = plt.xticks(rotation=30)

  ax.axes.set_title("Teacher/Course GTS comparison (2015-2019)",
                       fontsize=16)
   
  ax.set_xlabel("Employee ID",
                   fontsize=14)
   
  ax.set_ylabel("Percent Agree",
                  fontsize=14)
  
  ax.set_ylim([0, 105])
  plt.subplots_adjust(left=0.07, bottom=0.1, right=0.97, top=0.93)

  fig.savefig(folder+"teacher_gts_{:02d}.png".format(j))
