...
selected_course_code = input("Course_code: ")
...
def qry_get_course_info(year, semester, course_code, tbl='vw1_course_summaries_fixed', schema='ces'):
  # Returns a dataframe of the courses undergoing enhancement course in year, semester from db (cur)
  qry = " SELECT DISTINCT \n" \
        "   crse.level, cd.school_code, crse.course_code, \n" \
        "   crse.course_code_ces, \n" \
        "   crse.cluster_code, \n" \
        "   cd.school_code, cd.school_name, cd.course_name \n" \
        " FROM ( \n" \
        "   SELECT level, course_code, course_code_ces, cluster_code  \n" \
        "	  FROM {0}.{1} \n" \
        "   WHERE year = {2} AND semester = {3} AND course_code = '{4}'\n" \
        "   ) crse \n" \
        " LEFT OUTER JOIN ( \n" \
        "   SELECT * FROM lookups.vw_course_details_recent \n" \
        "   ) cd ON (SPLIT_PART(cd.course_code,'-', 1) = SPLIT_PART(crse.course_code,'-', 1))\n" \
        " ORDER BY cd.school_code, crse.course_code \n" \
        "".format(schema, tbl,
...
def get_course_list(year, semester, course_code, cur, schema='ces'):
  # Returns a dataframe of the courses undergoing enhancement course in year, semester from db (cur)
  qry = qry_get_course_info(year, semester, course_code, 'vw1_course_summaries_fixed', schema)
...
'''-------------------------------------------- Create Dataframes -------------------------------------'''
df_ce = get_course_list(year, semester, selected_course_code,
                        cur=postgres_cur)
