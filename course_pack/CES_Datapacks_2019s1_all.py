...
def qry_course_list(year, semester, tbl='vw2_course_summaries_fixed', schema='ces'):
  ...
  qry = " SELECT DISTINCT \n" \
        ...
        "   ces.course_code_ces, \n" \
        "   ces.school_name_short, cd.course_name \n" \
        " FROM ( \n" \
        "   SELECT level, school_code, course_code, course_code_ces, school_name_short  \n" \
        "	  FROM {0}.{1} \n" \
        "   WHERE year = {2} AND semester = {3} \n" \
        "   ) ces \n" \
        " LEFT OUTER JOIN ( \n" \
        "   SELECT * FROM lookups.vw_course_details_recent \n" \
        "   ) cd ON (SPLIT_PART(cd.course_code,'-', 1) = SPLIT_PART(ces.course_code,'-', 1))\n" \
        " ORDER BY ces.school_name_short, ces.course_code \n" \
        "".format(schema, tbl,
                  year, semester)
  return qry

def get_course_list(year, semester, cur, tbl='vw2_course_summaries_fixed', schema='ces'):
  # Returns a dataframe of the courses undergoing enhancement course in year, semester from db (cur)
...
df_crse_list = get_course_list(year, semester,
                                    cur=postgres_cur)
df_schools = df_crse_list[['school_code', 'school_name_short']].drop_duplicates()
...
def create_school_options():
  ...
  df_schools.sort_values(['school_name_short'])
          ...
          children=[
            ...
            html.Div(
              ...
              [
                ...
                html.P(['Each students answers the OSI once for the course.'],
                ...
                html.P(['The Course GTS percent agree is calculated by taking the sum of student responses'
                        ' to GTS questions that are "Agree" or "Strongly Agree" for any teacher and expressing '
                        ' it as a percentage of the total number of responses to GTS questions for the course.'],
                       style={'textAlign': 'left',
                              'font-size': 16,
                              'color': rc.RMIT_Black,
                              'font-weight': 'normal',
                ...
                html.P(['The GTS and OSI (percent agree) range from 0 to 100%.'],
                       style={'textAlign': 'left',
                              'font-size': 16,
                              'color': rc.RMIT_Black,
                              'font-weight': 'normal',
...
def make_header_div(df1):
  ...
  try:
    ...
    school_name = df1['school_name_short'].tolist()[0]
    div = html.Div(
      [
        # Left - Headings
        html.Div(
